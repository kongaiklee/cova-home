---
title: "Claims-Made vs Occurrence Triggers: Why It Matters Which Lines Use Which"
slug: "/comparison/claims-made-vs-occurrence"
category: "comparison"
...
>
> An **occurrence-based** policy responds to events that *occurred* during the policy period, regardless of when the claim is later notified. A **claims-made** policy responds to claims *first made* against the insured (and notified to the insurer) during the policy period, with cover bounded by a **retroactive date** (no cover for events before that date) and an **extended reporting period** option (cover for late-notified claims after expiry). In Singapore, **Public Liability, Property, WICA, and Motor are typically occurrence-based**; **Professional Indemnity, Directors & Officers, Cyber, and Employment Practices Liability are typically claims-made**. The structural difference matters most at policy cancellation, business sale, run-off, and when claims arise from past acts.
...
- WICA / Workers' Compensation
- Motor
- Marine Cargo (for individual shipments)
- Plate Glass
- Money
...
Claims-made was developed for liability lines where:
- The act causing loss may not be discovered for years (negligent advice manifesting as client loss after a delay)
...
The single most common claims-made mistake in Singapore SME insurance is the retroactive date trap. Three flavours:

1. **First-time PI / D&O / Cyber buyer with retroactive date set at policy inception.** Any claim arising from acts before that date is uncovered. Common when an SME first buys cover at, say, 1 Jan 2024, then in March receives a claim about advice given in 2023. Uncovered.

2. **Insurer change that resets the retroactive date.** Switching from Insurer A to Insurer B at renewal, with the new policy's retroactive date defaulting to the new inception date. Acts during Insurer A's period are now uncovered if claims arrive after expiry. Mitigation: negotiate "continuity of cover" with retroactive date matching the original inception with Insurer A.

3. **Acquisition resetting the retroactive date.** When an SME is acquired, the buyer's master D&O policy may not extend to acts of the target's directors before the acquisition. Run-off cover on the seller's policy is the standard solution.
...
1. **Cancelling a claims-made policy without buying tail.** Acts during the policy period are now uninsured for any claim notified after cancellation.
2. **First-time PI/D&O/Cyber buyer with no retroactive cover.** The retroactive date defaults to inception; any pre-policy act is uncovered.
3. **Switching insurer without continuity.** New policy's retroactive date resets to the new inception, leaving prior years uncovered for late-notified claims.
4. **Treating "claims-made" and "occurrence" as the same.** Founders sometimes assume the policy that's in force when the claim arrives is the one that pays. For occurrence-based, that's wrong. For claims-made, that's right but bounded by retroactive date.
5. **At M&A, forgetting D&O run-off.** Outgoing directors face personal liability for pre-deal acts; without tail, their D&O is silent.
6. **Notifying late on a claims-made policy.** Notification during the policy period (or ERP) is the trigger. Late notification = uncovered claim, even if the act was within cover.

### What This Means for Your Business

For SMEs holding a mix of occurrence-based and claims-made policies (which is most SMEs), the operational discipline that matters:

1. **Map your policies by trigger type.** Know which are occurrence-based (PL, Property, WICA, Motor) and which are claims-made (PI, D&O, Cyber, EPL). Not all brokers proactively explain this.
