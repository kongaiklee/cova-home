---
title: "How to Run an Insurance Tender for a Singapore SME: A Procedural Playbook for the 2026 Soft Market"
slug: "/procedural-howto/how-to-run-insurance-tender-sme-singapore"
...
Tenders cost time. The decision to run one should be deliberate.
...
For most Singapore SMEs with annual commercial premium between S$15,000 and S$500,000, a formal tender once every two or three renewals is the right cadence.
...
The tender brief is the single most important document in the process. A thin brief produces thin quotes. A comprehensive brief produces sharp ones.

The brief should be a single PDF or document pack containing seven sections.
...
Or two years of audited plus most recent unaudited management accounts. Revenue, gross profit, payroll, asset register, and balance sheet. The financial statements anchor the liability rating (rate-on-revenue), the business interruption sum insured (gross profit at risk over an indemnity period), and the property sum insured (asset values).

### 3. Five-Year Claims History

A claims history from each prior insurer is the document alternate markets need to price credibly. Without it, alternate quotes price defensively on worst-case loss-ratio assumptions and the SME loses leverage. Request a Claims Experience Report from each prior insurer covering the last five renewal cycles. The CER should show, by line: paid claims with date and short description, outstanding reserves, incurred-but-not-reported (IBNR) provisions, claims declined, and claims withdrawn.

Where the SME has fewer than five years of trading history, provide what exists and state the date of incorporation explicitly.

### 4. Current Programme Specification

A line-by-line description of the cover currently in place. For each policy: insurer, policy number, period, sum insured / limit of indemnity (per occurrence and aggregate as applicable), excess / deductible by peril, key extensions, key exclusions, current annual premium (gross, GST-inclusive), broker commission disclosed (or noted as not disclosed). Attach the existing policy schedules and key wording sections.

### 5. Exposure Data Pack
...
- **Cyber**: endpoint count, server count, cloud architecture, sensitive-data inventory, MFA coverage, backup-restore last-test date, incident-response plan status, previous cyber incidents (notifiable or non-notifiable).
- **Motor**: vehicle register, driver profile, business use category, claims history per vehicle.
- **Marine cargo**: trade flows, commodity classes, Institute Cargo Clauses preferred, average single sending and annual aggregate.
- **Professional indemnity**: services rendered, geographic split of revenue, contractual liability assumed, professional qualifications of senior fee earners, prior-acts position.
...
What you want quoted, and what response format you require. Specify:

- The lines to be quoted.
- The renewal date and required quote date (typically T-30 from renewal).
...
- [MAS Notice FAA-N02 — Appointment and Use of Introducers by Financial Advisers](https://www.mas.gov.sg/regulation/notices/notice-faa-n02). The notice under which introducers (including COVA) operate. Introducers may identify potential clients and pass them to licensed advisers; they may not advise, recommend, or arrange.
...
A reasonable response window for an SME composite tender is three to four weeks from brief release to quote return. Anything shorter pressures the responding firms to short-cut the underwriting; anything longer is a signal the SME is not serious.
...
Quote responses arrive as PDFs from each firm. The first step is normalising them into a single comparison grid. The grid should show, by line of cover:

| Line | Insurer | Sum Insured / Limit | Deductible | Premium (S$) | GST (S$) | Brokerage/Fee | Key Extensions | Key Exclusions |
|------|---------|---------------------|------------|--------------|----------|---------------|----------------|----------------|

Build the grid in a spreadsheet, not a Word table. A spreadsheet allows side-by-side delta calculations and total programme comparisons.
...
The award decision should be documented in a single-page memo recording:

- The panel invited and the responses received.
- The evaluation grid scored against the published criteria.
- The recommended firm and the reasons.
- The runner-up (because the runner-up frequently becomes the renewal challenger).
- Any conditions of award (e.g., specific extensions to be secured, specific wording amendments to be executed, broker remuneration disclosure to be provided in writing before binding).

The memo is the institutional record. If staff turnover affects the finance or operations team within the policy year, the memo is what the successor relies on.
